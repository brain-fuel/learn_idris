...
-- Test driver for the PPM-to-ASCII (`ic`) miniproject.
--
-- Spawns the solution as a subprocess, pipes the fixture stdin,
-- captures stdout, and asserts the expected lines appear.
--
-- Default target: ch12-cli/miniproject/solution.idr (the LEARNER stub).
-- Override with the SOLUTION env var to test the _key/ reference instead.
--
-- Run from the repo root:
--   idris2 --no-banner --exec main ch12-cli/t/SpecPPM.idr
--   SOLUTION=ch12-cli/miniproject/_key/solution.idr idris2 --no-banner --exec main ch12-cli/t/SpecPPM.idr

defaultSolution : String
defaultSolution = "ch12-cli/miniproject/solution.idr"

fixturePath : String
fixturePath = "ch12-cli/miniproject/fixtures/input.txt"

outPath : String
outPath = "/tmp/learn_idris_ch12_ic_driver_out.txt"
...
expected = ["wrote /tmp/learn_idris_ch12_ic_out.txt", "3x2"]
...
main = do
  ...
  _ <- system "rm -f /tmp/learn_idris_ch12_ic_out.txt"

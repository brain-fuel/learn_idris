...
-- Test driver for the read-aloud (text-to-speech) miniproject.
--
-- Spawns the solution as a subprocess, pipes the fixture stdin, captures
-- stdout, and asserts the expected lines appear.
--
-- Default target: ch24-tts/miniproject/solution.idr (the LEARNER stub).
-- Override with the SOLUTION env var to test the _key/ reference instead.
--
-- Run from the repo root:
--   idris2 --no-banner --exec main ch24-tts/t/SpecSpeak.idr
--   SOLUTION=ch24-tts/miniproject/_key/solution.idr idris2 --no-banner --exec main ch24-tts/t/SpecSpeak.idr

defaultSolution : String
defaultSolution = "ch24-tts/miniproject/solution.idr"

fixturePath : String
fixturePath = "ch24-tts/miniproject/fixtures/input.txt"

outPath : String
outPath = "/tmp/learn_idris_ch24_speech_out.txt"
...
expected = [ "wrote /tmp/learn_idris_ch24_speech.wav"
           , "RIFF"
           ]

setupFixture : IO ()
setupFixture = do
  _ <- System.system "rm -f /tmp/learn_idris_ch24_speech.wav /tmp/learn_idris_ch24_speech_out.txt"
  pure ()

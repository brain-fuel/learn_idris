...
-- Test driver for the Expense Report miniproject.
--
-- Spawns the solution as a subprocess, pipes the fixture stdin (a
-- single "go" trigger so getLine doesn't block), captures stdout, and
-- asserts the expected per-category lines + TOTAL appear.
--
-- Default target: ch14-spreadsheets/miniproject/solution.idr (the LEARNER stub).
-- Override with the SOLUTION env var to test the _key/ reference instead.
--
-- Run from the repo root:
--   idris2 --no-banner --exec main ch14-spreadsheets/t/SpecExpenses.idr
--   SOLUTION=ch14-spreadsheets/miniproject/_key/solution.idr idris2 --no-banner --exec main ch14-spreadsheets/t/SpecExpenses.idr

defaultSolution : String
defaultSolution = "ch14-spreadsheets/miniproject/solution.idr"

fixturePath : String
fixturePath = "ch14-spreadsheets/miniproject/fixtures/input.txt"

outPath : String
outPath = "/tmp/learn_idris_ch14_expenses_out.txt"
...
expected = ["food", "transport", "entertainment", "TOTAL", "267.75"]

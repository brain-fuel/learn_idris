...
-- Test driver for the Task Tracker miniproject.
--
-- Spawns the solution as a subprocess, pipes the fixture stdin, captures
-- stdout, and asserts the expected lines appear.
--
-- Default target: ch16-sqlite/miniproject/solution.idr (the LEARNER stub).
-- Override with the SOLUTION env var to test the _key/ reference instead.
--
-- Run from the repo root:
--   idris2 --no-banner --exec main ch16-sqlite/t/SpecTasks.idr
--   SOLUTION=ch16-sqlite/miniproject/_key/solution.idr idris2 --no-banner --exec main ch16-sqlite/t/SpecTasks.idr
--
-- External: sqlite3 CLI must be on PATH (setupFixture uses it).

defaultSolution : String
defaultSolution = "ch16-sqlite/miniproject/solution.idr"

fixturePath : String
fixturePath = "ch16-sqlite/miniproject/fixtures/input.txt"

outPath : String
outPath = "/tmp/learn_idris_ch16_tasks_out.txt"
...
expected = ["laundry", "dishes", "id=", "quit"]

setupFixture : IO ()
setupFixture = do
  _ <- System.system "rm -f /tmp/learn_idris_ch16_tasks.db"
  _ <- System.system "sqlite3 /tmp/learn_idris_ch16_tasks.db 'CREATE TABLE tasks(id INTEGER PRIMARY KEY, title TEXT, done INT);'"
  pure ()
